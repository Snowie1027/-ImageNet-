...
def initialize_model(num_classes=200):
...
# 训练和验证函数
def train_and_evaluate(train_loader, val_loader, lr_pretrained, lr_fc, num_epochs=20):
    ...
    # 创建日志目录
    log_dir = f'runs/lr_pretrained_{lr_pretrained}_lr_fc_{lr_fc}'
    os.makedirs(log_dir, exist_ok=True)
    writer = SummaryWriter(log_dir)

    print(f"Starting training with lr_pretrained={lr_pretrained}, lr_fc={lr_fc}")
...
for lr_pretrained, lr_fc in learning_rates:
    train_and_evaluate(train_loader, val_loader, lr_pretrained, lr_fc, num_epochs)
