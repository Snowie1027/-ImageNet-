...
def initialize_model(pretrained=False, num_classes=200):
...
# 学习率设置
learning_rate_high = 0.01  # 设置较高的学习率
num_epochs = 20

# 训练和验证函数
def train_and_evaluate(train_loader, val_loader, learning_rate, num_epochs=20, pretrained=False):
    model = initialize_model(pretrained=pretrained)
    optimizer = optim.SGD(model.parameters(), lr=learning_rate, momentum=0.9)
    scheduler = StepLR(optimizer, step_size=7, gamma=0.1)

    # 创建日志目录
    log_dir = f'runs/lr_{learning_rate}_pretrained_{pretrained}'
    os.makedirs(log_dir, exist_ok=True)
    writer = SummaryWriter(log_dir)

    print(f"Starting training with learning_rate={learning_rate}, pretrained={pretrained}")
    ...
    for epoch in range(num_epochs):
        ...
        writer.add_text('Learning Rate', f'learning_rate: {learning_rate}', epoch)
...
# 运行训练和验证
train_and_evaluate(train_loader, val_loader, learning_rate_high, num_epochs, pretrained=False)
